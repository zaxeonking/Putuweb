...
// lalu diteruskan ke TugasClient untuk di-seed ke Zustand store
// (store yang sama dipakai juga oleh Dashboard, Jadwal, & Kalender).
export const dynamic = 'force-dynamic';

export default async function TugasPage() {
  try {
    const { data } = await ambilDatabase();
    return <TugasClient initialDatabase={data} />;
  } catch (error) {
    ...
    return (
      <div className="space-y-6">
        ...
        <TugasClient initialDatabase={DATABASE_KOSONG} />
      </div>
    );
  }
}

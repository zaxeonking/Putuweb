...
// lalu diteruskan ke KalenderClient untuk di-seed ke Zustand store
// (store yang sama dipakai juga oleh Dashboard & Jadwal).
export const dynamic = 'force-dynamic';

export default async function KalenderPage() {
  try {
    const { data } = await ambilDatabase();
    return <KalenderClient initialDatabase={data} />;
  } catch (error) {
    ...
    return (
      <div className="space-y-6">
        ...
        <KalenderClient initialDatabase={DATABASE_KOSONG} />
      </div>
    );
  }
}
